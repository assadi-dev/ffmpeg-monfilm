	...
	try {
		...
		const promise = new Promise((resolve) => {
			...
			ffmpeg
				...
				.on("end", () => {
					console.log(`Finished audio concate`);
					logVideoProcess("Export video", `Finished complex filter concate audio`);
					status.progress = 100;
					status.message = "done";
					status.remain = 0;
					ws
	...
	try {
		...
		const promise = new Promise((resolve) => {
			ffmpeg
				...
				.outputOptions(["-map [out]", "-map 0:v", "-c:v", "libx264", "-c:a", "aac"])
				.output(destination)
				.on("start", (cmdline) => {
					//console.log(`start concate`, cmdline);
	...
	try {
		...
		const promise = new Promise((resolve) => {
			ffmpeg
				...
				.on("start", (cmdline) => {
					// console.log(`start concate`, cmdline);
					logVideoProcess("Export video", `Start file mapping no audio: ${cmdline}`);
					status.message = "start";
					ws
						.of(process.env.WEBSOCKET_PATH)
						.to(room)
				...
				.on("error", (error) => {
					...
					logErrorVideoProcess(
						"Export video",
						`Erreur file mapping no audio: ${error.message}`
					);
					clean_file_process(videoFile);
					status.message = "error";
					status.error = error.message;
