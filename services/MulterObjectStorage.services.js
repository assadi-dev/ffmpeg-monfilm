  ...
  getToken(callback) {
    ...
    request(
      ...
      (err, data, res) => {
        ...
        const serviceCatalog = _.find(data.token.catalog, {
          type: "object-store",
        });
        const endpoint = _.find(serviceCatalog.endpoints, {
        ...
        if (!endpoint) {
          ...
          return callback(
            new Error(
              `No endpoint for region ${
                this.config.region
              } found! Possible regions are: ${possibleRegions.join(",")}`
            )
          );
        }
