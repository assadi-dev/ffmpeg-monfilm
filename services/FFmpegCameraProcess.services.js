...
export const insv_equirectangular = async (fileObject) => {
	...
	try {
        ...
        console.log("Stderr output: " + stderrLine);
				...
				.on("end", () => {
					console.log(`Finished quirectangular insv processing for ${filename}`);
					logVideoProcess(
						"Traitement video",
					...
					const result = {
						filename: fileObject.finalFilename.replace("_dualfisheye.mp4", ".mp4"),
						output,
						duration: status.duration,
					};
					resolve(result);
	...
	try {
		...
		return new Promise((resolve) => {
			ffmpegCommand
				...
				.output(output)
				.on("start", (cmdline) => {
					console.log(`start equirectangular insv One X3 process for ${filename}`);
					logVideoProcess(
						"Traitement video",
        ...
        console.log("Stderr output: " + stderrLine);
				...
				.on("end", () => {
					console.log(`Finished equirectangular insv processing for ${filename}`);
					logVideoProcess(
						"Traitement video",
		...
		return new Promise((resolve) => {
			(async () => {
				...
				ffmpegCommand
					...
					.on("start", (cmdline) => {
						status.message = "start";
						logVideoProcess("Compression video", `Start compressing for ${input}`);
					...
					.on("end", () => {
						...
						logVideoProcess("Compression video", `Finished compressing for ${input}`);
						status.message = "done";
						status.progress = 100;
						ws.of(WEBSOCKET_PATH).to(room).emit("end", status);
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			ffmpeg.on("start", (cmdline) => {
				...
				logVideoProcess("Génération des thumbnail", `command ffmpeg: ${cmdline}`);
			});
			ffmpeg.on("end", async () => {
				console.log("Finish extract frame" + destination);
			...
			ffmpeg.on("error", (error) => {
				console.log(error);
				logErrorVideoProcess(`Génération des thumbnail","Erreur: ${error.message}`);
			});
			ffmpeg.run();
		} catch (error) {
			logErrorVideoProcess(`Génération des thumbnail","Erreur: ${error.message}`);
...
export const extract_duration = (input) => {
	const { ffmpeg } = new FFmpegInstance();
